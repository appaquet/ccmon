...
Canonical cwd, project name, session ID, source, backend identity, and protocol payloads remain unchanged.
...
- [ ] Implement workspace-aware display derivation and disambiguation (R12.A–R12.D; senior-dev)
  - AC: Distinct workspaces remain distinguishable without unnecessary session suffixes.
...
- [ ] Review and validate workspace identity (code-correctness reviewer + browser/manual validation)
  ...
  - AC: Same-host and cross-host workspace examples remain readable and distinct on wide and narrow layouts.
