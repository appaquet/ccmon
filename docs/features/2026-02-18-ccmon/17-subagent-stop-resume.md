...
- [x] Pass `stoppedAtMs` (from status file) into `getSubagentInfos()` — sub-agents with mtime <= stoppedAtMs + STOP_GRACE_MS are never considered active (R40)
- [x] Update `buildProjectState()` call site to pass the stopped timestamp (R40)
- [x] Add test: sub-agent mtime 30s ago, stopped at 10s ago → `isActive === false` (R40)
- [x] Add test: sub-agent mtime 5s ago, stopped at 10s ago → `isActive === true` (wrote after stop = new activity) (R40)
- [x] Add test: no stopped signal (null) → existing 45s threshold behavior unchanged (R40)
