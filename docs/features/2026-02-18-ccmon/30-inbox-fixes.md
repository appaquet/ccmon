...
- [ ] In `onclose`/`onerror` handler: clear `entry.projects = []` and call `mergeAndRender()` to update display immediately
- [ ] In `onopen` handler: clear `entry.projects = []` before server sends initial state (avoid brief stale window between connected status and first message)
...
- [ ] In `scanEnrichment()` in `src/sessions.ts`: add parsing for `Task` tool_use blocks — extract `tool_use.id` → `description` map
- [ ] In `scanEnrichment()`: parse `toolUseResult` entries for `agentId` — correlate `tool_use_id` back to tool_use `id` to get description, add to `agentDescriptions` map
- [ ] Keep existing `queue-operation` parsing as fallback for older sessions
- [ ] In UI (`public/index.html`): update fallback chain to `description ?? slug ?? agentId`
- [ ] Add test: Task tool_use + toolUseResult correlation populates agentDescriptions
- [ ] Add test: mixed queue-operation (legacy) and Task tool_use entries both populate descriptions
