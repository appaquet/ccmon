...
- [x] Add `flashWaitingDismissed` Set (module-level, keyed by `projKey`) alongside existing flash Maps (~line 673)
- [x] In `createCard()`: pass key as parameter; check `!flashWaitingDismissed.has(key)` before applying `card-flashing-waiting` class (~line 613)
