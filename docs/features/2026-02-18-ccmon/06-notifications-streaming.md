...
[ ] Extract latestAssistantMessage from JSONL — last type:"assistant" entry with text content, truncated to 200 chars; latestUserMessage already exists, confirm both populated (R28)
[ ] Add tests for latestAssistantMessage extraction and latestUserMessage pairing (R28)
...
[ ] Define shared SessionEnrichment base type: model, latestUserMessage, latestAssistantMessage, lastToolUse, tasksDone, tasksTotal (R29.1)
[ ] Define SubagentInfo extending SessionEnrichment: agentId, slug, jsonlPath; refactor ProjectState to extend SessionEnrichment (R29.1, R29.2)
[ ] Refactor countActiveSubagents into getSubagentInfos: returns SubagentInfo[], applies byte-offset streaming to each sub-agent JSONL (R29, R29.1)
[ ] Implement sub-agent stopped detection: check parent JSONL for matching Task tool_result, fall back to mtime heuristic (R29.3)
[ ] Add tests for shared type structure, sub-agent info extraction, and stopped detection (R29.1, R29.2, R29.3)
[ ] Update ProjectState type: replace subagentCount with subagents: SubagentInfo[] (R29)
